...
class K8s():
    ...
    def get_gpu(self,resource_gpu, resource_name=None):
        ...
        try:
            if resource_gpu:
                ...
                if gpu_type and gpu_type.lower() in list(self.gpu_resource.keys()):
    ...
    #         node_resource={}
    #         a = node.status
    #         node_resource['used_memory'] = self.to_memory_GB(node.status.allocatable["memory"])   # 只能获取总机器可分配额度
    #         node_resource['used_cpu'] = self.to_cpu(node.status.allocatable["cpu"])
    #         vgpu = float(node.status.allocatable.get('tencent.com/vcuda-core', '0')) / 100
    #         ai_resource = {}
    #         for name in self.gpu_resource:
    #             resource = self.gpu_resource[name]
    #             gpu = int(node.status.allocatable.get(resource, '0'))
    #             ai_resource["used_"+name] = gpu
    #         ai_resource['used_gpu'] = ai_resource.get('used_gpu', 0) + vgpu
    #         node_resource.update(ai_resource)
    #         nodes_resource[node.metadata.name]=node_resource
    #
        ...
        try:
            ...
            for pod in pods:
                ...
                for name in self.gpu_resource:
                    resource = self.gpu_resource[name]
                    gpu = [int(container.resources.requests.get(resource, '0')) for container in containers if container.resources and container.resources.requests]
                    ai_resource["used_"+name] = sum(gpu)
                ai_resource['used_gpu'] = ai_resource.get('used_gpu', 0) + sum(vgpu)

                nodes_resource[node_name].update(ai_resource)
    ...
    def make_container(self, name, command, args, volume_mount, working_dir, resource_memory, resource_cpu,
        ...
        # if -1==gpu_num:
        #     # 没要gpu的容器，就要加上可视gpu为空，不然gpu镜像能看到和使用所有gpu，默认就是能用所有卡
