...
import {
  IADUGTemplateActionItem,
  IAppMenuItem,
} from "../api/interface/kubeflowInterface";
import {
  getADUGTemplateList,
  getADUGTemplateApiInfo,
  actionADUGTemplateDelete,
  getADUGTemplateDetail,
  actionADUGTemplateAdd,
  actionADUGTemplateUpdate,
  actionADUGTemplateSingle,
  actionADUGTemplateMuliple,
  actionADUGTemplateRetryInfo,
  actionADUGTemplateFavorite,
  actionADUGTemplateCancelFavorite,
  actionADUGTemplateChartOption,
} from "../api/kubeflowApi";
import { ColumnsType } from "antd/lib/table";
import MixSearch, {
  IMixSearchParamItem,
} from "../components/MixSearch/MixSearch";
import DynamicForm, {
  calculateId,
  IDynamicFormConfigItem,
  IDynamicFormGroupConfigItem,
  ILinkageConfig,
...
export default function TaskListManager(props?: IAppMenuItem) {
  ...
  const [visableAdd, setVisableAdd] = useState(
    getParam("isVisableAdd") === "true" || false,
  );
  ...
  const [filterValues, _setFilterValues] = useState<
    Array<{ key: ReactText | undefined; value: ReactText | undefined }>
  >([]);
  const filterValuesRef = useRef(filterValues);
  const setFilterValues = (
    data: Array<{ key: ReactText | undefined; value: ReactText | undefined }>,
  ): void => {
    filterValuesRef.current = data;
    _setFilterValues(data);
  };
  const [dynamicFormConfigAdd, setDynamicFormConfigAdd] = useState<
    IDynamicFormConfigItem[]
  >([]);
  const [dynamicFormConfigUpdate, setDynamicFormConfigUpdate] = useState<
    IDynamicFormConfigItem[]
  >([]);
  const [dynamicFormGroupConfigAdd, setDynamicFormGroupConfigAdd] = useState<
    IDynamicFormGroupConfigItem[]
  >([]);
  const [dynamicFormGroupConfigUpdate, setDynamicFormGroupConfigUpdate] =
    useState<IDynamicFormGroupConfigItem[]>([]);
  // TODO: 子视图中需要额外手动禁用的字段，可在此制定，例: project: true, pipeline: true,
  const subViewDisabledFieldMap: Record<string, boolean> = {};
  ...
  const [dataDetail, setDataDetail] = useState<
    Array<{ label: string; value: any; key: string }>
  >([]);
  ...
  const [columnRelateFormat, setColumnRelateFormat] = useState<
    ILinkageConfig[]
  >([]);
  ...
  const createDyFormConfig = (
    data: Record<string, any>[],
    label_columns: Record<string, any>,
    ...
    return data.map((item, index) => {
      ...
      const list = createDyFormConfig(
        item.info || [],
        label_columns,
        description_columns,
      );

      // 如果 URL 上带了 id 并且当前字段的 values 里存在该 id，
      const urlIdStr = getParam("id");
      const urlId = urlIdStr ? parseInt(urlIdStr, 10) : undefined;
      // 使用 isSubRoute 判断是否为子路由，而不是通过 URL 参数
      const isSubView = props?.isSubRoute || false;
      ...
      const hasUrlDefault =
        ...
        (item.values as any[]).some((v: any) => v && v.id === urlId);

      const computedDefault = hasUrlDefault
        ? urlId
        : item.default === ""
          ? undefined
          : item.default;
      ...
      const res: IDynamicFormConfigItem = {
        ...
        list,
        name: item.name,
        // 子视图时：优先使用 parentViewName 禁用父视图字段，如果没有则使用 subViewDisabledFieldMap 手动禁用
        disable:
          item.disable ||
          (isSubView &&
            (item.name === parentViewName ||
              !!subViewDisabledFieldMap[item.name])),
        description:
          item.description || description_columns[item.name] || undefined,
        required: item.required,
        defaultValue: computedDefault,
        multiple: item["ui-type"] && item["ui-type"] === "select2",
        options: (item.values || []).map((item: any) => ({
          label: item.value,
          value: item.id,
    ...
    getADUGTemplateApiInfo(url)
      .then((res) => {
        ...
        const hasAction =
          ...
          permissions.includes("can_edit") ||
          permissions.includes("can_delete");
        const cacheColumns = localStorage.getItem(
          `tablebox_${location.pathname}`,
        );
        const cacheColumnsWidthMap = JSON.parse(cacheColumns || "[]").reduce(
          (pre: any, next: any) => ({ ...pre, [next.dataIndex]: next.width }),
          {},
        );

        const columnRelatedFormat: ILinkageConfig[] = Object.entries(
          column_related || {},
        ).reduce(
          (pre: any[], [key, value]) => [
            ...pre,
            {
              ...
              effectOption: value.related.reduce(
                (ePre: any, eNext) => ({
                  ...ePre,
                  [calculateId(eNext.src_value)]: eNext.des_value.map(
        ...
        const listColumns = list_columns
          .filter((column: any) => !fixed_columns.includes(column))
          .map((column) => {
            ...
            return {
              ...
              dataIndex: column,
              key: column,
              sorter: order_columns.includes(column)
              ...
              render: (text: any, record: any) => {
                  ...
                  if (typeValue && addedValue) {
                        ...
                        <Tooltip
                          title={
                            <span
                              ...
                              dangerouslySetInnerHTML={{ __html: addedValue }}
                            ></span>
                          }
                          placement="topLeft"
                        >
                          <div
                            className={cols_width[column].type || "ellip2"}
                    ...
                    if (typeValue === "enhancedDetails") {
                      return (
                        <div
                          onClick={() => {
                            setVisibleTabsModal(true);
                            setEnhancedDetailsUrl(addedValue);
                          }}
                          dangerouslySetInnerHTML={{ __html: text }}
                        ></div>
                      );
                ...
                if (
                  cols_width[column] &&
                  cols_width[column].type?.indexOf("ellip") !== -1
                ) {
                  return (
                    <Tooltip
                      title={
                    ...
                    >
                      <div
                        className={cols_width[column].type}
                        dangerouslySetInnerHTML={{ __html: text }}
                      ></div>
                    </Tooltip>
                  );
                ...
                if (
                  ...
                  const tarRes = Object.entries(text).reduce(
                    (pre: any, [label, value]) => [...pre, { label, value }],
                    [],
                  );
                  if (!tarRes.length) {
                    return "-";
                  ...
                  return (
                    <div style={{ overflow: "auto", maxHeight: 100 }}>
                      {tarRes.map((item: any, index: number) => {
                        return (
                          <div key={`table_itemvalue_${index}`}>
                            {label_columns[item.label] || item.label}:
                            {item.value}
                          </div>
                        );
                      })}
              ...
              width:
                cacheColumnsWidthMap[column] ||
                (cols_width[column] && cols_width[column].width) ||
                100,
            };
          });

        const fixedColumns: any = list_columns
          .filter((column: any) => fixed_columns.includes(column))
          .map((column) => {
            const columnTitle = label_columns[column] || column;
            const [before, after] = columnTitle.split(":");
        ...
        const actionList = Object.entries(action || {}).reduce(
          (pre: any, [name, value]) => [...pre, { ...value }],
          [],
        );
        const multipleAction: IADUGTemplateActionItem[] = actionList.filter(
          (item: any) => !!item.multiple,
        );
        const singleAction: IADUGTemplateActionItem[] = actionList.filter(
          (item: any) => !!item.single,
        );
              ...
              <Space size="middle">
                {hasAction ? (
                  <Dropdown
                    overlay={
                      <Menu>
                        {isShowCollectRef.current &&
                        isAllDataListRef.current ? (
                          <Menu.Item>
                            <div
                              className="link"
                              onClick={() => {
                                Modal.confirm({
                                  ...
                                  onOk() {
                                    return new Promise((resolve, reject) => {
                                      actionADUGTemplateFavorite(
                                        `${route_base}favorite/${record[primary_key]}`,
                                      )
                                        .then((res) => {
                                          resolve("");
                                        })
                              ...
                              onClick={() => {
                                ...
                                getADUGTemplateApiInfo(
                                  ...
                                  .then((res) => {
                                    ...
                                    const formConfigUpdate: IDynamicFormConfigItem[] =
                                      createDyFormConfig(
                                        edit_columns,
                                        label_columns,
                                        description_columns,
                                    ...
                                    edit_columns.forEach((item) => {
                                      if (item["ui-type"] === "list") {
                                        item.info.forEach((itemInfo: any) => {
                                          updateColumnsMap[itemInfo.name] =
                                            itemInfo;
                                        });
                                      }
                                    });
                                    setUpdateColumnsMap(updateColumnsMap);
                                    const formGroupConfigUpdate: IDynamicFormGroupConfigItem[] =
                                      edit_fieldsets.map((group) => {
                                        const currentData = group.fields
                                          .map(
                                            (field) => updateColumnsMap[field],
                                          )
                                          .filter((item) => !!item);
                                        const groupConfig = createDyFormConfig(
                                          currentData,
                                          label_columns,
                                          description_columns,
                                        );
                                    ...
                                    setDynamicFormConfigUpdate(
                                      formConfigUpdate,
                                    );
                                    setDynamicFormGroupConfigUpdate(
                        ...
                        {props?.related?.map((item, index) => {
                          return (
                            <Menu.Item key={`moreAction_${index}`}>
                              <div
                                className="link"
                                onClick={() => {
                                  // 通过 state 传递父视图的 model_name，避免在 URL 中显示
                                  const parentModelName =
                                    currentModelName || props?.model_name;
                                  navigate(
                                    `${location.pathname}/${item.name}?id=${record[primary_key]}`,
                                    {
                                      state: parentModelName ? { parent_view_name: parentModelName } : undefined
                                    }
                                  );
                                }}
                              >
                        ...
                        {!!singleAction.length &&
                          singleAction.map((action, index) => {
                            return (
                              <Menu.Item key={`table_action_${index}`}>
                                <div
                                  className="link"
                                  onClick={() => {
                                    action.icon === "url"
                                      ? window.open(
                                          `${route_base}action/${action.name}/${record[primary_key]}`,
                                          "_blank",
                                        )
                                      : Modal.confirm({
        ...
        const tarColumns: React.SetStateAction<ColumnsType<any>> = [
          ...listColumns,
        ];
        if (fixedColumns) {
        ...
        const addColumnsMap = add_columns.reduce(
          (pre: any, next: any) => ({ ...pre, [next.name]: next }),
          {},
        );
        if (customFormData && Object.keys(customFormData).length) {
          const reTryInfoQuene = (Object.keys(customFormData) || []).filter(
            (key) =>
              customFormData[key] &&
              addColumnsMap[key] &&
              addColumnsMap[key].retry_info,
          );
          ...
          const handleReTryInfo = (tar: string) => {
            ...
            actionADUGTemplateRetryInfo(`${route_base}_info`, {
              exist_add_args: tar,
            })
              .then((res) => {
                const {
                ...
                const formConfigAdd: IDynamicFormConfigItem[] =
                  createDyFormConfig(
                    add_columns,
                    label_columns,
                    description_columns,
                  );
                const formGroupConfigAdd: IDynamicFormGroupConfigItem[] =
                  add_fieldsets.map((group) => {
                    const currentData = group.fields
                      .map((field) => addColumnsMap[field])
                      .filter((item) => !!item);
                    const groupConfig = createDyFormConfig(
                      currentData,
                      label_columns,
        ...
        const updateColumnsMap = edit_columns.reduce(
          (pre: any, next: any) => ({ ...pre, [next.name]: next }),
          {},
        );
        edit_columns.forEach((item) => {
        ...
        const formConfigAdd: IDynamicFormConfigItem[] = createDyFormConfig(
          add_columns,
          label_columns,
          description_columns,
        );
        ...
        const tarFilter: IMixSearchParamItem[] = Object.entries(filters).reduce(
          (pre: any, [name, value]) => {
            return [
              ...pre,
              {
        ...
        const localCacheFilter = JSON.parse(
          localStorage.getItem(
            `filter_${location.pathname}${location.search}`,
          ) || "[]",
        );
        ...
        const currentTableWidth = cacheColumns
          ? tarColumns.reduce(
              (pre: any, next: any) => pre + next.width || 100,
              0,
            ) + 80
          : tarColumns.length * 100 + 80 + actionwidth;
      ...
      filters: [
        ...
        ...params
          ...
          .map((param: any) => {
            ...
            const sourceOprList: string[] =
              paramsMap[param.key].filter.map((item: any) => item.operator) ||
              [];
  ...
  const fetchData = (
    { pageConf, params, paramsMap, sorter, only_favorite }: fatchDataParams = {
      pageConf: pageInfoInit,
      params: filterValues,
      paramsMap: filterParamsMap,
      sorter: undefined,
    ...
    if (selectedRowKeys.length) {
      Modal.confirm({
        ...
        onOk() {
          return new Promise((resolve, reject) => {
            actionADUGTemplateMuliple(
              `${baseUrlRef.current}multi_action/${action.name}`,
              {
                ids: selectedRowKeys.map(
                  (item: any) => JSON.parse(item || "{}")[primaryKey],
                ),
              },
            )
              .then((res) => {
    ...
    <div className="fade-in h100 d-f fd-c">
        ...
        {(form: FormInstance, formChangeRes: any) => (
          <DynamicForm
            ...
            onRetryInfoChange={(value) => {
              ...
              actionADUGTemplateRetryInfo(`${baseUrlRef.current}_info`, {
                exist_add_args: tar,
              })
                .then((res) => {
                  const {
            ...
            config={dynamicFormConfigAdd}
            configGroup={dynamicFormGroupConfigAdd}
          />
        )}
      </ModalForm>
      {/* 修改 */}
      <ModalForm
            ...
            const value = (next.value || []).map((item: any) => {
              for (const listItemKey in item) {
                if (Object.prototype.hasOwnProperty.call(item, listItemKey)) {
                  ...
                  if (
                    (updateColumnsMap[listItemKey] || {})["ui-type"] ===
                    "datePicker"
                  ) {
        ...
        onCreate={(values) => {
          ...
          actionADUGTemplateUpdate(
            `${baseUrlRef.current}${values[primaryKey]}`,
            values,
          )
            .then((res) => {
      ...
      {/* tabs详情 */}
      <TabsModal
        visible={visibleTabsModal}
        url={enhancedDetailsUrl}
        onVisibilityChange={setVisibleTabsModal}
      />
      <TitleHeader
        title={
          <>
        ...
        breadcrumbs={(props?.breadcrumbs || []).map((crumbs, idx) => {
          return (
            <span
              key={`templateTitle_${props?.name}_${idx}`}
              className="c-icon-b fs12"
            >
              /<span className="plr2">{crumbs}</span>
            </span>
          );
        })}
      >
            ...
            <TableBox
              ...
              buttonNode={
                <div className="d-f ac">
                  ...
                  {multipleAction && multipleAction.length ? (
                    <div>
                      <Dropdown
                        overlay={
                          <Menu>
                            {multipleAction.map((action, index) => {
                              return (
                                <Menu.Item key={`table_muliple_${index}`}>
                                  <span
                                    className="link"
                        ...
                        title={
                          <span className="tips-content-b">
                            ...
                            <div
                              ...
                              onClick={() => {
                                window.open(
                                  `${window.location.origin}${baseUrlRef.current}download_template`,
                                );
                              }}
                            >
                              {t("下载导入模板")}
                ...
                {dataList.map((row, rowIndex) => {
                  return (
                    <div
                      style={{
                        overflowY: "auto",
                        width: list_ui_args?.card_width,
                        height: list_ui_args?.card_height,
                      }}
                      key={`card${rowIndex}`}
                      className="card-row mr16 mb16"
                    >
