...
def upgrade():
    ...
    op.add_column('docker', sa.Column('project_id', sa.Integer(), nullable=False, comment='项目组id'))
    op.create_foreign_key(None, 'docker', 'project', ['project_id'], ['id'])
    op.add_column('notebook', sa.Column('expand', sa.Text(length=65536), nullable=True, comment='扩展参数'))
