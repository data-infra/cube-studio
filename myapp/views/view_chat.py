...
'''.strip()
    ...
    @expose('/file/<chat_name>/<file_name>', methods=['POST', 'GET'])
    # @pysnooper.snoop(depth=3,watch_explode=('aa',))
    def brower_file(self, chat_name, file_name=None):
        chat = db.session.query(Chat).filter_by(name=chat_name).first()
        files=[]
        if chat:
            file_paths = json.loads(chat.knowledge).get("file", '')
        ...
        if '.csv' in des_file:
            ...
            save_html_path = '/temp/'+'table.html'
            # 将 HTML 表格保存为文件
            with open(save_html_path, 'w') as f:
                f.write(table)
                f.close()
            # 返回 HTML 文件
            return send_file(save_html_path)
        if '.doc' in des_file:
            import mammoth
            # 读取 DOC 文件
            with open(des_file, 'rb') as docx_file:
                result = mammoth.convert_to_html(docx_file)
                html = result.value
            # 将 HTML 文件保存为文件
            save_html_path = '/temp/' + 'document.html'
            with open(save_html_path, 'w') as f:
                f.write(html)
                f.close()
            # 返回 HTML 文件
        ...
        if '.md' in des_file:
            import markdown
            # 读取 MD 文件
            with open(des_file, 'r') as md_file:
